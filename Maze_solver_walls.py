...
class Screen:
    ...
    def __init__(self):
        ...
        self.visited_reward = -0.1
        self.unvisited_reward = 0.2
        self.total_reward = 0
        self.learning_rate = 0.1
        self.q_table = np.zeros((200, 200, 2, 5))
    ...
    def viable_actions(self):
        ...
        fires = fire(loc_fire)
        loc_fires = np.zeros(5)
        if len(fires) >= 1:
            for i in fires:
                loc_fires[Screen.q_table_reverse_mapping[i]] = 1
        # np.reshape(directions_available, (3, 3))
        ...
        return viable_actions, loc_fires
    ...
    def action(self, viable_actions, fires):
        ...
        # Select the action with the highest q
            ...
            for i in range(len(fires)):
                np.append(options, self.q_table[self.actor[1], self.actor[0], int(fires[i]), i])
        ...
        #np.reshape(fires, (3, 3))

        self.q_table[old_actor[1], old_actor[0], int(fires[Screen.q_table_reverse_mapping[action]]), Screen.q_table_reverse_mapping[action]] += self.learning_rate * \
                            (resultant_reward + self.discount_factor * max_q - self.q_table[old_actor[1], old_actor[0], int(fires[Screen.q_table_reverse_mapping[action]]), Screen.q_table_reverse_mapping[action]])
